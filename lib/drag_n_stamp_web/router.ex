defmodule DragNStampWeb.Router do
  use DragNStampWeb, :router

  pipeline :browser do
    plug :accepts, ["html"]
    plug DragNStampWeb.Plugs.DomainRedirect
    plug :fetch_session
    plug :fetch_live_flash
    plug :put_root_layout, html: {DragNStampWeb.Layouts, :root}
    plug :protect_from_forgery
    plug :put_secure_browser_headers
    plug DragNStampWeb.Plugs.ExtensionHeaders
    plug DragNStampWeb.Plugs.Auth
  end

  pipeline :api do
    plug :accepts, ["json"]
    plug DragNStampWeb.Plugs.CORS
  end

  scope "/", DragNStampWeb do
    pipe_through :browser

    live "/", HomeLive
    live "/feed", FeedLive
    live "/leaderboard", LeaderboardLive
    live "/more-info", MoreInfoLive

    # Static extension endpoint (no LiveView/WebSockets)
    get "/extension", PageController, :extension

    # Temporary redirect from legacy /seo paths
    get "/seo/:filename", PageController, :legacy_submission_redirect

    # Dynamic submission fallback to render timestamp details when static files are missing
    get "/submissions/:filename", SeoPageController, :show

    # SEO endpoints
    get "/sitemap.xml", PageController, :sitemap
  end

  # Authentication routes
  scope "/auth", DragNStampWeb do
    pipe_through :browser

    get "/:provider", AuthController, :request
    get "/:provider/callback", AuthController, :callback
    delete "/logout", AuthController, :delete
  end

  # API routes
  scope "/api", DragNStampWeb do
    pipe_through :api

    post "/receive-url", ApiController, :receive_url
    options "/receive-url", ApiController, :receive_url
    post "/gemini", ApiController, :gemini
    options "/gemini", ApiController, :gemini
  end

  # Enable LiveDashboard and Swoosh mailbox preview in development
  if Application.compile_env(:drag_n_stamp, :dev_routes) do
    # If you want to use the LiveDashboard in production, you should put
    # it behind authentication and allow only admins to access it.
    # If your application does not have an admins-only section yet,
    # you can use Plug.BasicAuth to set up some basic authentication
    # as long as you are also using SSL (which you should anyway).
    import Phoenix.LiveDashboard.Router

    scope "/dev" do
      pipe_through :browser

      live_dashboard "/dashboard", metrics: DragNStampWeb.Telemetry
      forward "/mailbox", Plug.Swoosh.MailboxPreview
    end
  end
end
